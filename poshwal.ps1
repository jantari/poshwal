...
if ($Apply) {
    ...
    if ($wt = Get-AppxPackage -Name 'Microsoft.WindowsTerminal') {
        ...
        if ($wtCfgFile.Count -eq 1) {
            ...
            $wtSettings.profiles | Where-Object { $_.colorScheme } | ForEach-Object {
                $_.colorScheme = 'Generated by poshwal'
            }
        
            $wtSettings | ConvertTo-Json -Depth 100 | Set-Content -Path $wtCfgFile.FullName
        } else {
            Write-Warning "$($wtCfgFile.Count ) configuration files for the new Windows Terminal found, skipping. You are probably running a version of wt that is not supported yet."
        }
    }
